...
(function() {
  var $, Config, HeaderView, KeyConfig, KeyConfigSet, KeyConfigSetView, KeyConfigView, WebFontConfig, fk, keyCodes, keys, marginBottom, resizeTimer, saveData, scHelp, windowOnResize;
  ...
  KeyConfigView = Backbone.View.extend({
    ...
    setHelp: function() {
      var help, i, keycombo, lang, mode, ol, _i, _ref, _results;
      this.$("td.desc").empty();
      if ((mode = this.model.get("mode")) === "simEvent") {
        ...
        if (help) {
          if (help[lang].length > 1) {
            ol = this.$("td.desc")[0].appendChild(document.createElement("ol"));
            _results = [];
            for (i = _i = 0, _ref = help[lang].length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
              _results.push(ol.appendChild(document.createElement("li")).textContent = help[lang][i]);
            }
            return _results;
          } else {
            return this.$("td.desc").text(help[lang][0]);
          }
        }
      }
    },
  ...
  KeyConfigSetView = Backbone.View.extend({
    ...
    setCanvasHeader: function() {
      var ctx, fillText, fx, fy;
      fx = -76;
      fy = 120;
      fillText = function(ctx, text) {
