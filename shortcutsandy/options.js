...
(function() {
  var $, BookmarkOptionsView, BookmarksView, CommandOptionsView, CommandsView, Config, CtxMenuFolder, CtxMenuFolderSet, CtxMenuGetterView, CtxMenuManagerView, CtxMenuOptionsView, ExplorerBaseView, HeaderView, KeyConfig, KeyConfigSet, KeyConfigSetView, KeyConfigView, OptionExtProgView, PopupBaseView, Router, SettingsView, WebFontConfig, andy, bmOpenMode, catnames, commandsDisp, ctxMenuManagerView, decodeKbdEvent, defaultSleep, endEdit, escape, gMaxItems, getUuid, keyCodes, keyConfigSetView, keys, langs, lastFocused, loading, marginBottom, modeDisp, modifierInits, modifierKeys, resizeTimer, router, scHelp, scHelpSect, startEdit, tmplCtxMenus, transKbdEvent, windowOnResize, _ref, _ref1, _ref2, _ref3, _ref4,
  ...
  PopupBaseView = Backbone.View.extend({
    ...
    onShowPopup: function(name, model) {
      ...
      this.el.style.left = Math.round((window.innerWidth - this.el.offsetWidth) / 2) + "px";
      this.el.style.top = Math.max(0, Math.round((window.innerHeight - this.el.offsetHeight) / 2)) + "px";
      this.$(".caption").focus();
      $(".backscreen").show();
      return true;
    },
  ...
  ExplorerBaseView = (function(_super) {
    ...
    ExplorerBaseView.prototype.onClickExpandAll = function() {
      if (this.$(".expandAll").is(":checked")) {
        ...
        this.$(".folder[data-id='1']").addClass("expanded");
      }
  ...
  SettingsView = (function(_super) {
    ...
    SettingsView.prototype.render = function() {
      ...
      this.$(".sleepMSec").val(this.model.get("defaultSleep"));
    ...
    SettingsView.prototype.onSubmitForm = function() {
      var defaultSleep;
      defaultSleep = ~~this.$(".sleepMSec").val();
  ...
  OptionExtProgView = (function(_super) {
    __extends(OptionExtProgView, _super);

    function OptionExtProgView() {
      _ref = OptionExtProgView.__super__.constructor.apply(this, arguments);
      return _ref;
    }

    OptionExtProgView.prototype.name = "optionExtProg";

    OptionExtProgView.prototype.el = ".optionExtProg";

    OptionExtProgView.prototype.events = _.extend({
      "change input[name='program']": "onChangeProgram"
    }, PopupBaseView.prototype.events);

    OptionExtProgView.prototype.render = function() {
      var radio, value;
      this.$(".progPath").val("");
      if (this.options = this.model.get("command")) {
        value = this.options.content;
        if ((radio = this.$("input[name='program'][value='" + value + "']")).length === 0) {
          this.$(".progPath").val(value);
          value = "other";
        }
      } else {
        value = "iexplore";
      }
      return this.$("input[name='program'][value='" + value + "']")[0].checked = true;
    };

    OptionExtProgView.prototype.onChangeProgram = function(event) {
      if (this.$("input[name='program'][value='other']").is(":checked")) {
        return this.$(".progPath").focus();
      } else {
        return this.$(".progPath").blur();
      }
    };

    OptionExtProgView.prototype.onSubmitForm = function() {
      var caption, content;
      if ((content = this.$("input[name='program']:checked").val()) === "other") {
        caption = content = $.trim(this.$(".progPath").val());
      } else {
        caption = $.trim(this.$("input[name='program']:checked").parent().text());
      }
      if (content !== "") {
        this.model.set({
          "command": _.extend({
  ...
  CommandOptionsView = (function(_super) {
    ...
    CommandOptionsView.prototype.onShowPopup = function(name, model, commandName) {
  ...
  CommandsView = (function(_super) {
    ...
    CommandsView.prototype.onSubmitForm = function() {
      ...
      if (command = this.$(".radioCommand:checked").val()) {
        if (command === "openExtProg") {
          this.trigger("showPopup", "optionExtProg", this.model.id);
  ...
  BookmarkOptionsView = (function(_super) {
    ...
    function BookmarkOptionsView() {
      _ref2 = BookmarkOptionsView.__super__.constructor.apply(this, arguments);
      return _ref2;
    }
    ...
    BookmarkOptionsView.prototype.render = function() {
      var elFindtab, findtab, url, _ref3, _ref4;
      if (this.newSite) {
        this.options = this.newSite;
      }
      this.$(".bookmark").text(this.options.title);
      url = this.options.url;
      if (/^javascript:/i.test(url)) {
        $.each(this.$(".inputs").children(), function(i, elem) {
          return $(elem).hide();
        });
        ...
        this.$(".bookmark").css("background-image", "-webkit-image-set(url(chrome://favicon/size/16@1x/" + this.options.url + ") 1x)");
      }
      if (this.options.url.length > 1024) {
        this.$(".url").text(url.substring(0, 1024) + " ...");
      } else {
        this.$(".url").text(url);
      }
      this.$(".findStr").val(this.options.findStr || url);
      if ((_ref3 = this.$("input[value='" + (this.options.openmode || 'current') + "']").get(0)) != null) {
        _ref3.checked = true;
      }
      this.$(".tabpos").val("last");
      if ((_ref4 = this.options.openmode) === "left" || _ref4 === "right" || _ref4 === "first" || _ref4 === "last") {
  ...
  BookmarksView = (function(_super) {
    ...
    function BookmarksView() {
      _ref3 = BookmarksView.__super__.constructor.apply(this, arguments);
      return _ref3;
    }
  ...
  CtxMenuOptionsView = (function(_super) {
    ...
    function CtxMenuOptionsView() {
      _ref4 = CtxMenuOptionsView.__super__.constructor.apply(this, arguments);
      return _ref4;
    }
    ...
    CtxMenuOptionsView.prototype.render = function() {
      var container, contexts, ctxType, desc, lastParentId, parentId, selectParent$, _ref5, _ref6;
    ...
    CtxMenuOptionsView.prototype.onClickSubmit = function(event) {
      var caption, container, ctxType, dfd, model, parentId, parentName, _ref5, _ref6,
        _this = this;
      ...
      if (/delete/.test(event != null ? event.currentTarget.className : void 0)) {
        ...
        if (parentId === "route") {
          ...
          this.model.set("ctxMenu", {
            caption: caption,
            parentId: parentId,
            order: (_ref6 = this.ctxMenu) != null ? _ref6.order : void 0
          });
        }
      }
  ...
  CtxMenuManagerView = (function(_super) {
    ...
    CtxMenuManagerView.prototype.onUpdateFolder = function(event, ui, view) {
      ...
      if (ui && _.filter(ui.item.parent().find(".title"), function(title) {
        var _ref5;
        return title.textContent === ((_ref5 = ui.item.prevInfo) != null ? _ref5.text : void 0);
      }).length > 1) {
    ...
    CtxMenuManagerView.prototype.onClickDone = function() {
      ...
      $.each(this.$(".ctxMenuItem"), function(i, el) {
        var contexts, menu$, message, _ref5;
        menu$ = $(el);
        message = {
          id: el.id
        };
        ...
        message.order = i + 1;
        message.parentId = ((_ref5 = menu$.parents(".folder").get(0)) != null ? _ref5.id : void 0) || "route";
    ...
    CtxMenuManagerView.prototype.onClickNew = function(event) {
      var className, contexts$, entried, target$, _ref5,
        _this = this;
      if (!/contexts|title/.test((_ref5 = (target$ = $(document.activeElement)).get(0)) != null ? _ref5.className : void 0)) {
        return;
      }
      this.activeFolder = {};
  ...
  transKbdEvent = function(value) {
    var i, keyCombo, keyIdenfiers, modifiers, scanCode, _i, _ref5;
    modifiers = parseInt(value.substring(0, 2), 16);
    keyCombo = [];
    for (i = _i = 0, _ref5 = modifierInits.length; 0 <= _ref5 ? _i < _ref5 : _i > _ref5; i = 0 <= _ref5 ? ++_i : --_i) {
  ...
  KeyConfigView = Backbone.View.extend({
    ...
    onKeydown: function(event) {
      var container, i, keyname, keynames, scCode, scanCode, _i, _ref5, _ref6;
      if ((_ref5 = event.target.tagName) === "TEXTAREA" || _ref5 === "INPUT" || _ref5 === "SELECT") {
        return;
      }
      ...
      if (keynames = keyIdentifiers[this.kbdtype][event.originalEvent.keyIdentifier]) {
        ...
        for (i = _i = 0, _ref6 = keys.length; 0 <= _ref6 ? _i < _ref6 : _i > _ref6; i = 0 <= _ref6 ? ++_i : --_i) {
    ...
    onClickEdit: function(event) {
      ...
      switch (mode) {
        ...
        case "command":
          if (this.model.get("command").name === "openExtProg") {
            return this.trigger("showPopup", "optionExtProg", this.model.id);
          } else {
    ...
    onClickDelete: function() {
      ...
      if (parentId = this.model.get("parentId")) {
        switch (this.model.get("mode")) {
          case "remap":
            desc = "Shortcut key: " + decodeKbdEvent(this.model.get("origin")) + ("\n Description: \"" + (this.getDescription()) + "\"");
            break;
          case "sleep":
            desc = "Sleep " + this.model.get("sleep") + " msec";
            break;
          default:
            desc = "Description: \"" + (this.getDescription()) + "\"";
        }
        children = [];
    ...
    setDesc: function() {
      var bookmark, command, commandDisp, content, content3row, desc, editOption, help, i, key, keycombo, lines, mode, pause, sleep, tdDesc, test, _i, _j, _ref5, _ref6, _ref7;
      ...
      switch (mode) {
        ...
        case "command":
          ...
          if (commandDisp[2]) {
            ...
            lines = ((_ref5 = command.content) != null ? _ref5.split("\n") : void 0) || [];
        ...
        case "disabled":
          ...
          if (help　 && help[this.lang]) {
            for (i = _j = 0, _ref7 = help[this.lang].length; 0 <= _ref7 ? _j < _ref7 : _j > _ref7; i = 0 <= _ref7 ? ++_j : --_j) {
  ...
  KeyConfigSetView = Backbone.View.extend({
    ...
    onEnterCtxMenuSelMode: function(entried) {
      ...
      this.collection.models.forEach(function(model) {
        var _ref5;
        return model.trigger("setUnselectable", (_ref5 = model.id, __indexOf.call(entried, _ref5) >= 0) ? true : false);
      });
    ...
    onSetSaveData: function(newData) {
      ...
      this.collection.models.forEach(function(model) {
        var command;
        if (model.get("mode") === "command") {
          command = model.get("command");
          if (command.name === "execJS" && command.coffee) {
            return andy.coffee2JS(model.get("new"), command.content);
          }
        }
      });
      andy.remakeCtxMenu(this.getSaveData());
      loading = false;
      return windowOnResize();
    },
    ...
    onKeyDown: function(event) {
      var elActive, i, keyname, keynames, scCode, scanCode, _i, _ref5, _ref6;
      if (!this.model.get("singleKey")) {
        return;
      }
      if ((elActive = document.activeElement) && ((_ref5 = elActive.tagName) === "TEXTAREA" || _ref5 === "INPUT" || _ref5 === "SELECT")) {
        return;
      }
    ...
    redrawTable: function() {
      ...
      return $.each(this.$("tbody > tr"), function(i, tr) {
        var tr$, _ref5;
        if (!/child/.test((_ref5 = tr.nextSibling) != null ? _ref5.className : void 0)) {
