...
> >
> > ショートカットキー to ショートカットキーも可能なので、Chromeが標準で備えているショートカットキーのキーアサインを  
...
>     任意のキーを押下して登録します。
> > 
> >    - Originがショートカットキーで、Chromeのショートカット機能が割り当てられている場合は、Descriptionに説明が表示されます。
...
> - **Descriptionの編集（メモ機能）**  
>    「Edit Description」を選択します。 Remap/Disabledモード時に、Descriptionが空欄のときに選択できます。  
>    メモ的な用途で利用します。
> - **コンテキストメニューへの登録**  
>    「Create/Edit context menu」を選択します。 [コンテキストメニュー登録・編集](#ctxMenuOptions)ダイアログが開きます。
> - **バッチ実行の追加キーイベントの登録**  
>    「Add KeyEvent」を選択します。 詳細は、[バッチ実行機能](#batch)を参照してください。
...
> 
> また、拡張コマンドのInject scriptで利用できる、UtilityObjectの*ctxData*プロパティの値にコンテキストの情報がセットされ  
...
>    テキスト選択(Selection)のコンテキストメニューの場合、文字列中に「%s」を挿入した箇所が、選択中のテキストに置き換わります。
> - **Target Context**  
...
>    入力テキストを選択ハイライトしている場合のみ、選択テキストが返されます。
>    - **Link**  
...
>
> なお、同じTargetに2つ以上登録した場合は、自動的にサブメニューにまとめられます。 これはChrome拡張のコンテキストメニューの仕様です。
...
> ###### `tsc.ctxData(string)`  
> > 
> > コンテキストメニューから呼ばれた場合に、コンテキストの情報がセットされます。
