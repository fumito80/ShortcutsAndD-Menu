...
> > ショートカットキーを指定した場合は、リマップ定義済みのショートカットキーでもリマップされません。
> > 
> >    引数の書式は_send_メソッドと同じです。 単独キーの場合、角括弧は必要ありません。
> > 
> > Code Exsample  
> > 
...
> > </pre>
> > 
> > 次のコードは、ブックマークバーのその他のブックマークを開きます。（ブックマークバーが隠れている状態から）
> > <pre>
> > tsc.keydown('[cs]b');
> > tsc.keydown('F6');
> > tsc.keydown('F6');
