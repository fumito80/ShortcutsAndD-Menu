...
(function() {
  var analyzeScHelpPage, closeTabs, closeWindow, deleteHistory, dfdCommandQueue, execBatch, execCommand, execShortcut, flexkbd, getActiveTab, getAllTabs, getAllTabs2, getWindowTabs, jsUitlObj, notifications, openBookmark, optionsTabId, preOpenBookmark, preSendKeyEvent, scHelp, scHelpPageUrl, scHelpSect, scrapeHelp, sendKeyEventToDom, sendMessage, setClipboardWithHistory, setConfigPlugin, showCopyHistory, showNotification,
    ...
    if (transCode) {
      ...
      if (!execMode) {
        for (i = _k = 0, _ref1 = local.keyConfigSet.length; 0 <= _ref1 ? _k < _ref1 : _k > _ref1; i = 0 <= _ref1 ? ++_k : --_k) {
          if ((item = local.keyConfigSet[i])["new"] === scCode) {
            execMode = item.mode;
            break;
          }
        }
  ...
  chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (tab.url.indexOf(chrome.extension.getURL("options.html")) === 0 && (changeInfo.status = "complete")) {
