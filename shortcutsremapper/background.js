...
(function() {
  var analyzeScHelpPage, closeTabs, closeWindow, createCtxMenus, deleteHistory, dfdCommandQueue, execBatch, execCommand, execCtxMenu, execShortcut, flexkbd, getActiveTab, getAllTabs, getAllTabs2, getWindowTabs, jsCtxData, jsUtilObj, modifierInits, notifications, openBookmark, optionsTabId, preOpenBookmark, registerCtxMenu, registerCtxMenuParent, removeCookie, removeCtxMenu, scHelp, scHelpPageUrl, scHelpSect, scrapeHelp, sendMessage, setClipboardWithHistory, setConfigPlugin, showCopyHistory, showNotification, tabStateNotifier, transKbdEvent,
  ...
  execShortcut = function(dfd, doneCallback, transCode, sleepMSec, execMode, batchIndex) {
    var i, item, kbdtype, keyIdentifier, keys, modifierChars, modifiers, modifiersCode, scCode, scanCode, test, _i, _j, _k, _ref, _ref1, _results;
    if (transCode) {
      scCode = "";
      modifiersCode = 0;
  ...
  execCtxMenu = function(info) {
    var i, keyConfig, keydownMode, transCode, _i, _ref;
    ...
    if (transCode) {
      return execShortcut($.Deferred(), (function(dfd) {
        return dfd.resolve();
      }), transCode, 0, keydownMode);
    }
  };
  ...
  window.andy = {
    ...
    remakeCtxMenu: function(saveData, spec) {
      var dfd,
        _this = this;
      dfd = $.Deferred();
  ...
  registerCtxMenu = function(dfd, ctxMenus, index) {
    var caption, contexts, ctxData, ctxMenu, id, parent, type, _ref;
    if (ctxMenu = ctxMenus[index]) {
      _ref = ctxMenus[index], id = _ref.id, type = _ref.type, caption = _ref.caption, contexts = _ref.contexts, parent = _ref.parent;
      if (/pause/.test(type)) {
        ctxData = {
          type: "normal",
          enabled: false
  ...
  registerCtxMenuParent = function(dfd, ctxMenus, index) {
    var ctxData, ctxMenu;
    if (ctxMenu = ctxMenus[index]) {
      ctxData = {
        type: "normal",
      ...
      return chrome.contextMenus.create(ctxData, function() {
        return registerCtxMenuParent(dfd, ctxMenus, index + 1);
      });
    } else {
      return dfd.resolve();
    }
  ...
  removeCtxMenu = function(dfd, ctxMenus, index) {
    var ctxMenu;
    if (ctxMenu = ctxMenus[index]) {
      return chrome.contextMenus.remove(ctxMenu.id, function() {
        return removeCtxMenu(dfd, ctxMenus, index + 1);
      });
    } else {
      return dfd.resolve();
    }
  ...
  createCtxMenus = function() {
    var dfdMain, keyConfigSet;
    if (keyConfigSet = andy.local.keyConfigSet) {
      dfdMain = $.Deferred();
      chrome.contextMenus.removeAll(function() {
        var ctxMenuParentNames, ctxMenuParents, ctxMenus, dfd, key;
        ctxMenus = [];
        ctxMenuParentNames = {};
        keyConfigSet.forEach(function(keyConfig) {
          ...
          if ((ctxMenu = keyConfig.ctxMenu)) {
            ...
            if (ctxMenu.parent !== "root") {
              return ctxMenuParentNames[ctxMenu.parent] = ctxMenu.parentOrder || 999;
            }
          }
        });
        ctxMenuParents = [];
        for (key in ctxMenuParentNames) {
          ctxMenuParents.push({
            id: key,
            order: ctxMenuParentNames[key]
          });
        }
        ctxMenuParents.sort(function(a, b) {
