...
(function() {
  var analyzeScHelpPage, closeTabs, dfdCommandQueue, execBatch, execCommand, execShortcut, flexkbd, forecast, getActiveTab, getAllTabs, getAllTabs2, getWindowTabs, jsUitlObj, notifDisplay, notification, openBookmark, optionsTabId, preOpenBookmark, preSendKeyEvent, scHelp, scHelpPageUrl, scHelpSect, scrapeHelp, sendKeyEventToDom, sendMessage, setClipboardWithHistory, setConfigPlugin, showCopyHistory, showNotification, xhr,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  flexkbd = document.getElementById("flexkbd");

  execShortcut = function(dfd, doneCallback, transCode, sleepMSec, batchIndex) {
    var i, item, kbdtype, keyIdentifier, keys, local, modeExec, modifierChars, modifiers, modifiersCode, scCode, scanCode, test, _i, _j, _k, _ref, _ref1, _results;
    if (transCode) {
      scCode = "";
        ...
        switch (request.action) {
          ...
          case "pasteText":
            notification.onclose = function() {
              var notifDisplay;
              notifDisplay = false;
              return flexkbd.PasteText(request.value1);
            };
            doneCallback(dfd, 0);
        }
      } catch (_error) {
  ...
  showNotification = function() {
    ...
    notification.ondisplay = function() {
      return notifDisplay = true;
    };
    notification.onclose = function() {
      return notifDisplay = false;
    };
    notification.show();
    return notifDisplay = true;
  };
