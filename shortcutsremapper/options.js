...
(function() {
  var $, BookmarkOptionsView, BookmarksView, CommandOptionsView, CommandsView, Config, CtxMenuOptionsView, HeaderView, KeyConfig, KeyConfigSet, KeyConfigSetView, KeyConfigView, PopupBaseView, WebFontConfig, bmOpenMode, bookmarksView, catnames, commandOptionsView, commandsDisp, commandsView, ctxMenuIcon, ctxMenuOptionsView, decodeKbdEvent, endEdit, escape, fk, gMaxItems, keyCodes, keyConfigSetView, keys, lastFocused, marginBottom, modeDisp, modifierInits, modifierKeys, resizeTimer, saveData, scHelp, scHelpSect, startEdit, transKbdEvent, windowOnResize, _ref, _ref1,
  ...
  BookmarkOptionsView = (function(_super) {
    ...
    BookmarkOptionsView.prototype.onShowPopup = function(name, model, options) {
      if (!BookmarkOptionsView.__super__.onShowPopup.call(this, name, model, options)) {
        return;
      }
      return startEdit();
    };
  ...
  CtxMenuOptionsView = (function(_super) {
    ...
    function CtxMenuOptionsView() {
      _ref1 = CtxMenuOptionsView.__super__.constructor.apply(this, arguments);
      return _ref1;
    }
    ...
    CtxMenuOptionsView.prototype.render = function() {
      var selectParent$, _ref2, _ref3;
      this.ctxMenu = this.model.get("ctxMenu");
      this.$(".caption").val(((_ref2 = this.ctxMenu) != null ? _ref2.caption : void 0) || this.options.desc);
      this.$el.append(this.tmplHelp(this));
      this.$("input[value='" + (((_ref3 = this.ctxMenu) != null ? _ref3.contexts : void 0) || 'page') + "']")[0].checked = true;
      if (this.ctxMenu) {
        this.$(".delete").removeClass("disabled").removeAttr("disabled");
      } else {
        this.$(".delete").addClass("disabled").attr("disabled", "disabled");
      }
      if ((selectParent$ = this.$(".selectParent")).val() === "new") {
        selectParent$.val("root");
        return this.$(".parentName").attr("disabled", "disabled");
      }
    };

    CtxMenuOptionsView.prototype.onShowPopup = function(name, model, options) {
      if (!CtxMenuOptionsView.__super__.onShowPopup.call(this, name, model, options)) {
        return;
      }
      return startEdit();
    };
    ...
    CtxMenuOptionsView.prototype.onClickSelectParent = function(event) {
      if (event.currentTarget.value !== "new") {
        return this.$(".parentName").attr("disabled", "disabled").blur();
      } else {
        return this.$(".parentName").removeAttr("disabled").focus();
      }
    };
    ...
    CtxMenuOptionsView.prototype.setContextMenu = function(actionType, id, caption, contexts, callback) {
      var sendData;
      if (callback == null) {
        callback = function() {};
      }
    ...
    CtxMenuOptionsView.prototype.onClickSubmit = function(event) {
      var actionType, callback, caption, menutype;
      if ((caption = this.$(".caption").val()) !== "") {
        ...
        this.setContextMenu(actionType, this.model.id, caption, menutype, callback.bind(this));
      }
      return false;
    };

    CtxMenuOptionsView.prototype.hidePopup = function() {
      endEdit();
      return CtxMenuOptionsView.__super__.hidePopup.call(this);
    };
  ...
  transKbdEvent = function(value) {
    var i, keyCombo, keyIdenfiers, modifiers, scanCode, _i, _ref2;
    modifiers = parseInt(value.substring(0, 2), 16);
    keyCombo = [];
    for (i = _i = 0, _ref2 = modifierInits.length; 0 <= _ref2 ? _i < _ref2 : _i > _ref2; i = 0 <= _ref2 ? ++_i : --_i) {
  ...
  KeyConfigView = Backbone.View.extend({
    ...
    onKeydownOrigin: function(event) {
      var i, keyname, keynames, scCode, scanCode, _i, _ref2;
    ...
    setDesc: function() {
      var bookmark, command, commandDisp, content, content3row, desc, editOption, help, i, key, keycombo, lang, lines, mode, pause, tdDesc, test, _i, _j, _ref2, _ref3;
      ...
      switch (mode) {
        ...
        case "disabled":
          ...
          if (help) {
            for (i = _j = 0, _ref3 = help[lang].length; 0 <= _ref3 ? _j < _ref3 : _j > _ref3; i = 0 <= _ref3 ? ++_j : --_j) {
      ...
      if (mode === "disabled") {
        this.$(".addKey,.copySC,.seprater.1st,ctxmenu").remove();
      }
  ...
  KeyConfigSetView = Backbone.View.extend({
    ...
    onStopSort: function() {
      ...
      $.each(this.$("tbody tr"), function(i, tr) {
        var target$, _ref2, _ref3;
        if (tr.className === "data") {
          if (((_ref2 = $(tr).next("tr")[0]) != null ? _ref2.className : void 0) !== "border") {
            return $(tr).after(_this.tmplBorder);
          }
        } else {
          if (((_ref3 = (target$ = $(tr).next("tr"))[0]) != null ? _ref3.className : void 0) !== "data") {
