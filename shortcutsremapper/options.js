...
(function() {
  var $, BookmarkOptionsView, BookmarksView, CommandOptionsView, CommandsView, Config, CtxMenuFolder, CtxMenuFolderSet, CtxMenuGetterView, CtxMenuManagerView, CtxMenuOptionsView, EditableBaseView, ExplorerBaseView, HeaderView, KeyConfig, KeyConfigSet, KeyConfigSetView, KeyConfigView, PopupBaseView, WebFontConfig, andy, bmOpenMode, bookmarksView, catnames, commandOptionsView, commandsDisp, commandsView, ctxMenuManagerView, ctxMenuOptionsView, decodeKbdEvent, endEdit, escape, gMaxItems, getUuid, headerView, keyCodes, keyConfigSetView, keys, lastFocused, marginBottom, modeDisp, modifierInits, modifierKeys, resizeTimer, scHelp, scHelpSect, startEdit, tmplCtxMenus, transKbdEvent, windowOnResize, _ref, _ref1, _ref2, _ref3,
  ...
  getUuid = function() {
    ...
    return [S4(), S4()].join("") + (new Date / 1000 | 0);
  };
  ...
  KeyConfigSetView = Backbone.View.extend({
    ...
    onLeaveCtxMenuSelMode: function(cancel) {
      ...
      return this.$("tbody").selectable("destroy").sortable("enable").find("div.unselectable").removeClass("unselectable").end().find("tr.unselectable").removeClass(removeClass);
    },
    onClickAddKeyConfig: function(event) {
      var newItem$;
      if (this.$(".addnew").length > 0) {
