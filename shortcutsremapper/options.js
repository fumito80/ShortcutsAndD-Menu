...
(function() {
  ...
  KeyConfigView = Backbone.View.extend({
    ...
    onClickCopySC: function(event) {
      var body, command, desc, keycombo, method, text;
      ...
      if (/copySC/.test(event.currentTarget.className)) {
        ...
        method = "keydown";
        desc = this.$(".desc").find(".content").text();
      }
      if (desc) {
        desc = " " + desc;
      }
      body = "tsc." + method + "('" + (transKbdEvent(this.model.id)) + "');";
  ...
  KeyConfigSetView = Backbone.View.extend({
    ...
    onAddRender: function(model) {
      ...
      this.$("tbody").append(newChild = keyConfigView.render(this.model.get("kbdtype")).$el).append(this.tmplBorder);
