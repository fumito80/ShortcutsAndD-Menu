...
(function() {
  var $, BookmarkOptionsView, BookmarksView, CommandOptionsView, CommandsView, Config, CtxMenuFolder, CtxMenuFolderSet, CtxMenuGetterView, CtxMenuManagerView, CtxMenuOptionsView, EditableBaseView, ExplorerBaseView, HeaderView, KeyConfig, KeyConfigSet, KeyConfigSetView, KeyConfigView, PopupBaseView, WebFontConfig, andy, bmOpenMode, bookmarksView, catnames, commandOptionsView, commandsDisp, commandsView, ctxMenuManagerView, ctxMenuOptionsView, decodeKbdEvent, defaultSleep, endEdit, escape, gMaxItems, getUuid, headerView, keyCodes, keyConfigSetView, keys, lastFocused, marginBottom, modeDisp, modifierInits, modifierKeys, resizeTimer, scHelp, scHelpSect, startEdit, tmplCtxMenus, transKbdEvent, windowOnResize, _ref, _ref1, _ref2, _ref3,
  ...
  KeyConfigView = Backbone.View.extend({
    ...
    onClickUpDownChild: function(event) {
      var models, order,
        _this = this;
      order = -1;
      $.each((models = this.model.collection.where({
        parentId: this.model.get("parentId")
      })), function(i, model) {
      ...
      if (event.currentTarget.title === "up" && order > 0) {
        this.$el.parent()[0].insertBefore(this.el, this.el.previousSibling);
        return this.trigger("updateChildPos");
      } else if (event.currentTarget.title === "down" && models.length > (order + 1)) {
        this.$el.parent()[0].insertBefore(this.el, this.el.nextSibling.nextSibling);
        return this.trigger("updateChildPos");
      }
    },
